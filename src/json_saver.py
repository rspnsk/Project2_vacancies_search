...
class JSONSaver(JsonSaveAbstract):
    ...
    def get_vacancies(self):
        """Метод запроса вакансий. Открывает JSON-файл для чтения. Загружает содержимое файла как JSON.
           Преобразует каждую запись из JSON в объект Vacancy (через to_dict). Возвращает список объектов Vacancy."""
    ...
    def save_vacancies(self, vacancy):
        """Метод сохранения вакансии в файл. Получает все вакансии из файла. Преобразует их в словари.
